...
#include "Eigen/Dense"
#include "list"
#include "polytope/common_type_name.hpp"
#include "polytope/ellipsoid.hpp"
#include "polytope/line_segment.hpp"
#include "vector"
...
class Polytope {
  ...
  public:
    ...
    static vec_E<Vec3f> SortPtsInClockWise(vec_E<Vec3f>& pts, Vec3f normal) {
        ...
        std::sort(pts_valued.begin(),
                  pts_valued.end(),
                  [](const std::pair<double, Vec3f>& i, const std::pair<double, Vec3f>& j) {
    ...
    void Visualize(const ros::Publisher& pub,
        ...
        if (flag)
            vertexMarker.action = visualization_msgs::Marker::DELETEALL;
        else {
        ...
        if (flag)
            edgeMarker.action = visualization_msgs::Marker::DELETEALL;
        else {
